...
#
# 2026-09-10: 균등 배분을 걷어내고 **설치 1명당 월수익**에 맞춰 다시 나눴다.
# 하우스애드 슬롯은 한 번에 한 앱만 나오는 제로섬 자원인데, 그때까지 수익 0원인 앱과
# 732원인 앱이 같은 확률로 나오고 있었다. Play Console 최근 30일 기준:
#
#     ReadFocus     351 설치 / 257,000원 = 732원   <- 1위인데 설치가 351개뿐
#     FloatNote     456 설치 / 122,000원 = 268원
#     AutoStart   1,850 설치 / 279,000원 = 151원
#     FloatTimer    411 설치 /  50,300원 = 122원
#     Clipboard   3,670 설치 / 443,000원 = 121원
#     VolumeBooster 154 설치 /   3,240원 =  21원
#     FloatCalc     319 설치 /   6,380원 =  20원
#     PhotoCleaner   28 설치 /       0원 =   0원
#
# 최고와 최저가 35배다. 이 배분으로 하우스애드 설치 1건의 기대 수익이
# 179원(균등) -> 394원으로 2.2배가 된다.
#
# ARPU 순위를 그대로 쓰지는 않았다. 하우스애드는 "전환은 되는데 트래픽이 없는 앱"에
# 써야 효율이 가장 높다 — Clipboard(3,670 설치)와 AutoStart(1,850 설치)는 자체 유입이
# 이미 있어 ARPU 대비 덜 주고, ReadFocus 는 ARPU 1위이면서 설치가 351개뿐이라 더 줬다.
#
# 미검증 가정이 하나 있다: 클릭률이 앱마다 같다고 보고 짰다. ReadFocus 는 난독증·ADHD
# 보조 도구라 VolumeBooster 사용자에게는 클릭률이 낮을 수 있다. ARPU 격차(35배)가
# 클릭률 격차보다 훨씬 커서 방향은 맞지만, 정확한 배분은 openStore 에 referrer 를 붙여
# 설치 귀속을 측정한 뒤에 다시 잡을 것.
#
# 고치고 이 스크립트를 다시 돌린 뒤 사이트를 배포하면 끝이다. 앱 재출시가 필요 없다는
# 것이 원격 카탈로그로 옮긴 실익이다.
#
# 가중치는 노출 추첨뿐 아니라 앱이 아이콘을 받아 가는 순서에도 적용된다는 점에 유의할 것
# (FamilyPromo.refreshBlocking 이 같은 추첨을 쓴다) — 캐시가 덜 찬 초반에 더 세게 작용한다.
# 그래서 신규 사용자에게는 명목 배수보다 훨씬 세게 걸린다. ReadFocus 40 은 "3.2배"가
# 아니라 초반 한동안 거의 독점에 가깝게 나온다는 뜻이다.
WEIGHTS = {
    "readfocus":     40,
    "floatnote":     20,
    "autostart":     12,
