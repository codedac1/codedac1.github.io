...
#
# 지금은 의도적으로 비워 두었다 — 모든 형제 앱을 동일 비율로 노출한다.
#
# 특정 앱을 더 밀고 싶으면 여기에 {"slug": 20} 처럼 넣고 이 스크립트를 다시 돌린 뒤
# 사이트를 배포하면 된다. 앱 재출시가 필요 없다는 것이 원격 카탈로그로 옮긴 실익이다.
# (한 번 써 본 적이 있다: 번들 시절 카탈로그가 7개 앱에 고정돼 있어 FloatNote/
# PhotoCleaner 가 어디에서도 홍보되지 못했기에 잠시 두 배를 줬다.)
#
